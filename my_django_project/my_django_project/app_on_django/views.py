...
class BaseListView(View):
    paginate_by = 10
    template_name = 'quotes_and_autors_app/quotes_pagination.html'
...
class AuthorView(DetailView):
    model = Authors
    template_name = 'quotes_and_autors_app/author.html'
...
class AddAuthorView(CreateView):
    ...
    success_url = reverse_lazy('quotes_and_autors_app:main')


@method_decorator(login_required, name='dispatch')
class AddQuoteView(CreateView):
    model = Quotes
    template_name = 'quotes_and_autors_app/adding_quote.html'
...
class AddTagView(CreateView):
    model = Tags
    template_name = 'quotes_and_autors_app/adding_tag.html'
...
class BindTagView(CreateView):
    model = Tags
    template_name = 'quotes_and_autors_app/bind_tags.html'
...
def sync(request):
    task.delay()
    return redirect('quotes_and_autors_app:main')
